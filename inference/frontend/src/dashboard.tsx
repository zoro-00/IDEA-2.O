...
import { Play, Shield, Activity, RefreshCw, Layers, BarChart3, TrendingUp, CheckCircle, XCircle, AlertCircle, ChevronDown, ChevronUp, Search, X, Maximize2, Minimize2 } from 'lucide-react';
import './index.css';
...
const NODE_COLORS: Record<string, string> = { STABLE: '#64748B', SUSPICIOUS: '#EA580C', CRITICAL: '#DC2626', CONFIRMED_FRAUD: '#DC2626' };
...
const TypeBadge: FC<{ type: string }> = ({ type }) => {
  const cls = type === 'Circular' ? 'circular' : type === 'Layering' ? 'layering' : type === 'Structuring' ? 'structuring' : type === 'Dispersion' ? 'circular' : type === 'Gathering' ? 'circular' : 'normal';
  const icon = type === 'Circular' || type === 'Dispersion' || type === 'Gathering' ? <RefreshCw size={9} /> : type === 'Layering' ? <Layers size={9} /> : type === 'Structuring' ? <BarChart3 size={9} /> : <TrendingUp size={9} />;
...
export default function App() {
  ...
  const [focusedCase, setFocusedCase] = useState<{sender:string;receiver:string;tx_id:string;tx_type?:string;cycle_path?:string[];cycle_edges?:string[];relatedNodes?:string[];relatedEdges?:string[]}|null>(null);

  // Build pattern-aware focus highlight from a transaction row
  const buildFocusFromTx = useCallback((tx: TxRow, gd: GraphData) => {
    const base = { sender: tx.sender, receiver: tx.receiver, tx_id: tx.tx_id, tx_type: tx.tx_type, cycle_path: [] as string[], cycle_edges: [] as string[], relatedNodes: [] as string[], relatedEdges: [] as string[] };
    const resolveId = (n: string | GraphNode) => typeof n === 'string' ? n : n.id;
    if (tx.tx_type === 'Gathering') {
      // Fan-In: find all links targeting the hub (receiver)
      ...
      // Fan-Out: find all links from the source (sender)
      ...
      // Pass-through chain: inbound to sender + outbound from sender
  ...
  useEffect(() => {
    (window as any).graphRef = graphRef.current;
  }, [graphData]);
  ...
  useEffect(() => { if (graphRef.current && isLoaded) { graphRef.current.d3Force('charge').strength(-80); graphRef.current.d3Force('link').distance(40); graphRef.current.d3Force('center').strength(0.2); } }, [isLoaded, graphData.nodes.length]);
  ...
  const runDemo = () => {
    ...
    setGraphData(prev => ({
      nodes: prev.nodes.map(n => ({ ...n, status: 'STABLE', is_fraud: false })),
      links: []
    }));
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === 'inference_start') { setProgress({ processed: 0, total: msg.data.total }); }
        ...
        const row: TxRow = { tx_id: d.tx_id, sender: d.sender, receiver: d.receiver, amount: d.amount || 0, tx_type: d.tx_type || 'Normal', risk_score: d.risk_score || 0, currency: d.currency || 'USD', payment_format: d.payment_format || 'Wire', timestamp: d.timestamp || 0, status: isAlert ? 'SUSPICIOUS' : 'NORMAL', case_id: d.case_id, isNew: true, reasons: d.reasons || [], is_fraud_gt: d.is_fraud_gt };
        ...
        // Removed aggressive zoom on alert to allow user to see graph building smoothly
  ...
  useEffect(() => {
    focusedCaseRef.current = focusedCase;
  }, [focusedCase, graphData.nodes]);
  ...
  const nodeCanvasObject = useCallback((node: GraphNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    if (isCycleNode) {
      // Pulsing red ring for cycle nodes
      ...
      // Bright blue ring for sender/receiver
      ...
      // Amber glow for hub constellation nodes (Gathering/Dispersion/Layering)
      ...
      // Normal node (dimmed if a case is focused)
  ...
  return (
    <div className="app-root">
      ...
      <header className="app-header">
        ...
        <div className="header-right">
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginRight: '8px' }}>
            ...
            <input type="range" min="0.10" max="0.90" step="0.01" value={threshold} onChange={e => setThreshold(parseFloat(e.target.value))} style={{ width: 80, cursor: 'pointer' }} />
          </div>
      ...
      {/* Dev Controls removed from here and moved to header */}

      {/* Body — unified 60/40 layout */}
      <div className="main-content">
        {/* Left Panel: swaps content based on active tab */}
        <div className="table-panel">
          {activeTab === 'dashboard' ? (
                ...
                <table className="data-table">
                  <thead>
                    <tr>
                      ...
                      <th>Sender</th>
                      <th>Receiver</th>
                      <th className={`align-right ${sortCol === 'amount' ? 'sorted' : ''}`} onClick={() => handleSort('amount')}>Amount <SortIcon col="amount" sortCol={sortCol} sortDir={sortDir} /></th>
                      <th style={{ width: 70 }}>Currency</th>
                      <th style={{ width: 80 }}>Format</th>
                      <th style={{ width: 100 }}>Type</th>
                      <th className={`align-right ${sortCol === 'risk' ? 'sorted' : ''}`} onClick={() => handleSort('risk')} style={{ width: 110 }}>Risk Score <SortIcon col="risk" sortCol={sortCol} sortDir={sortDir} /></th>
                  ...
                  <tbody ref={tableBodyRef}>
                    {filteredTx.map((t, i) => {
                      ...
                      return (
                      <tr key={t.tx_id} className={`${t.status !== 'NORMAL' ? 'row-alert' : ''} ${t.isNew ? 'row-new' : ''} ${isSelected ? 'selected' : ''}`} onClick={() => {
                        if (isSelected) {
                          setSelectedTx(null);
                          setFocusedCase(null);
                        } else {
            ...
            /* Cases Tab — left panel shows alerts list */
            <div className="table-container">
              {cases.length === 0 ? (
        ...
        {/* Right: Graph + Case Review — always visible */}
        <div className="graph-panel">
          <div className="panel-header">
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              ...
              <button className="panel-toggle" onClick={() => {
                setIsFullscreen(f => !f);
                setTimeout(() => graphRef.current?.zoomToFit(600, 60), 100);
              }}>
          ...
          <div ref={containerRef} className={`graph-container ${graphCollapsed ? 'collapsed' : ''}`} style={isFullscreen ? { position: 'fixed', top: 64, left: 0, right: 0, bottom: 0, zIndex: 9999, background: 'var(--color-bg-primary)', display: 'flex' } : {}}>
            {isLoaded && !graphCollapsed && (
              <ForceGraph2D
                ref={graphRef as any} graphData={graphData} width={dimensions.width} height={dimensions.height}
                backgroundColor="transparent" nodeId="id" nodeCanvasObject={nodeCanvasObject}
                linkColor={(l: GraphLink) => {
                  ...
                  return l.is_alert ? 'rgba(239,68,68,0.75)' : 'rgba(255,255,255,0.2)';
                }}
                ...
                linkDirectionalParticleWidth={3} linkDirectionalParticleSpeed={0.006}
            ...
            <div className="case-review">
              <div className="case-review-header">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button className="close-btn" onClick={() => { setSelectedTx(null); setFocusedCase(null); graphRef.current?.zoomToFit(600, 60); }}><X size={14} /></button>
                </div>
              </div>
              <div className="case-details">
                ...
                <div className="detail-item"><div className="detail-label">Timestamp</div><div className="detail-value mono">{selectedTx.timestamp ? new Date(selectedTx.timestamp * 1000).toLocaleString() : 'N/A'}</div></div>
