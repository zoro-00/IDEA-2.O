...
class TGNN(torch.nn.Module):
    def __init__(self, num_features, num_gnn_layers, n_classes=2, 
                n_hidden=100, edge_updates=False, edge_dim=None, 
        ...
        # Integrates GNN spatial embeddings (sender, receiver, edge) with raw temporal attributes (amount and velocity)
        self.mlp = nn.Sequential(
            Linear(n_hidden * 3 + edge_dim, 128),
            nn.ReLU(),
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        x_src_dst = x[edge_index.T].reshape(-1, 2 * self.n_hidden).relu()
        x_combined = torch.cat((x_src_dst, edge_attr.view(-1, edge_attr.shape[1]), raw_edge_attr), dim=-1)
